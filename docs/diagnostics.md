...
| `K2D-M004` | warning | An **HS small material asks for G0 more than 20x its unload/reload shear modulus G_ur**. The model permits at most that ratio, so the run uses the capped G0 and says which value it used. |
...
| `K2D-M005` | note | A phase asks for the **small-strain history to be reset** (`phases[].resetsmall`, see `docs/k2d-format.md`). The note says how many stress points were cleared, so the phase meets the soil at G0 rather than at the stiffness the earlier phases degraded it to; stress, shear hardening and the preconsolidation pressure are carried over untouched. If the model contains **no** HS-small material, the note says so instead: the flag was read and honoured, but there was no history to clear. |
...
| `K2D-A003` | retired | **No longer raised.** It warned that a structural element standing on a node driven by a non-zero prescribed displacement did not receive the imposed motion, so its M, Q and N understated the action. The analysis has carried the motion into every structural element since 2026-08-13, and the one report that still lost it — the anchor force, which reported 0 where the solve carried EA·d/L — now reads it too. Measured against closed forms: a two-span plate whose middle support settles (M_B = 3EIδ/L² with the shear term) and an anchor between two driven edges and from a driven edge to a fixed point (N = EA·d/L). The code is kept here so that it is never given another meaning. |
...
| `K2D-A007` | note | A phase applies only **part of its staged change** (`phases[].mstage` < 1). The configuration the phase describes is not reached — the remainder is still carried by the soil, and the results belong to the partial stage. |
| `K2D-A008` | note | A phase **ignores undrained behaviour** for a material that declares Undrained (A) or (B): it is solved drained, so no new excess pore pressure is generated in it, and the excess pore pressure generated in earlier phases is kept as it is. Strength parameters are unchanged. The answer is long-term (or state-setting), not short-term. |
...
| `K2D-A010` | warning | A **consolidation or fully-coupled phase meets a flow barrier it cannot read**: an active plate or interface declared impermeable or semi-permeable (`flow_barrier` ≠ 0) on a line the mesh was not split along — a plate without interfaces, or a wall or interface that fell back to bonded (`K2D-G009`). With no seam there is no second pore equation for the declaration to act on, so water crosses the line as if the soil were continuous and a cut-off holds back less head than it would in the ground. A wall with interfaces, or an interface on mesh edges, is split and read (fully permeable ties the two sides, impermeable keeps them apart, semi-permeable is refused), and raises nothing. The steady groundwater-flow calculation splits the mesh along any barrier line on mesh edges, a plate without interfaces included. |
...
| `K2D-G015` | refusal | A **material-factored design approach (EC7 DA1-C2 / DA3) was asked for on a model containing a Hoek-Brown material**. The partial factors divide c′ and tan(φ′); this model has neither, so the rock would be solved at its CHARACTERISTIC strength while the report said the design approach had been applied — a wrong verdict, not a conservative approximation. EN 1997-1 gives no partial factor for a Hoek-Brown envelope. Apply the factors to an equivalent c′/φ′ — a Mohr-Coulomb line fitted to the Hoek-Brown envelope over the confining-stress range of the problem — on a Mohr-Coulomb material, or use a resistance-factored approach (EC7 DA2, TBDY 2018), which does not touch the material. |
| `K2D-G016` | refusal | A **prestressed anchor is active in a Safety (strength reduction) analysis** — the initial Safety procedure or a Safety phase. The strength-reduction search re-solves the ground from the unstressed state with the phase's structural elements in it, and a lock-off force belongs to a ground that has already moved: on the unstressed mesh it would pull on soil that carries no stress yet, and the factor of safety would depend on that. Plates, geogrids, embedded beams, interfaces and anchors without prestress take part in the search (KV-STR-010). Deactivate the prestressed anchor in the Safety phase to obtain the factor of safety without it. *Until 2026-09 this code refused every active structural element*, because the search was handed none of them: measured on the Griffiths & Lane slope, an active geogrid, anchor, plate carrying 150 kN/m/m and embedded beam each returned the factor of safety of the same mesh with the element deactivated, bit for bit. |
...
| `K2D-A018` | note | A **Safety run has active structural elements**, and they take part in the factor of safety: an interface's strength is reduced with the soil's, while a plate's, anchor's, geogrid's or embedded beam's own capacity is not. The search re-solves the phase from the unstressed state with the elements present from the start, so each also carries what the ground's settlement under its own weight does to it — including settlement that, in the phases built before it, may have happened before the element was installed. No structural force is reported for the Safety phase, because the state the search stops at is not the state those phases built. |
