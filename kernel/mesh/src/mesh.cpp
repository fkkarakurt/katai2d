...
Mesh refine_uniform(const Mesh& coarse) {
    const int npe = coarse.nodes_per_element;
    if (coarse.element_count <= 0 || (npe != 6 && npe != 15)) return Mesh{};

    // Which of the parent's nodes become CORNERS of the children: its three corners and
    // its three edge midpoints. For tri6 those are local 3,4,5; for tri15 the middle of
    // each edge trio, local 4, 7 and 10 (tri15.hpp's ordering). Every other node of a
    // tri15 parent -- the quarter-point and interior nodes -- is regenerated by the
    // builder below and must NOT be carried over, or it would linger as an unconnected
    // node and leave the system singular.
    const int mid_local[3] = {npe == 15 ? 4 : 3, npe == 15 ? 7 : 4, npe == 15 ? 10 : 5};

    // Compact the used coarse nodes into triangulation vertices, keeping node sharing:
    // the same coarse node index always maps to the same vertex, so children of
    // neighbouring parents meet exactly -- this is what makes the result nested.
    std::vector<int> vertex_of(coarse.node_count, -1);
    Triangulation tg;
    const auto vertex = [&](int node) {
        int& v = vertex_of[node];
        if (v < 0) {
            v = static_cast<int>(tg.x.size());
            tg.x.push_back(coarse.x[node]);
            tg.y.push_back(coarse.y[node]);
        }
        return v;
    };

    tg.triangles.reserve(static_cast<size_t>(coarse.element_count) * 4);
    for (int e = 0; e < coarse.element_count; ++e) {
        const int c0 = vertex(coarse.node_of(e, 0));
        const int c1 = vertex(coarse.node_of(e, 1));
        const int c2 = vertex(coarse.node_of(e, 2));
        const int m01 = vertex(coarse.node_of(e, mid_local[0]));
        const int m12 = vertex(coarse.node_of(e, mid_local[1]));
        const int m20 = vertex(coarse.node_of(e, mid_local[2]));
        // Four children, each counter-clockwise like the parent: three at the corners
        // and the middle triangle on the midpoints.
        tg.triangles.push_back({c0, m01, m20});
        tg.triangles.push_back({m01, c1, m12});
        tg.triangles.push_back({m20, m12, c2});
        tg.triangles.push_back({m01, m12, m20});
    }
    tg.point_count = static_cast<int>(tg.x.size());

    Mesh fine = npe == 15 ? tri15_from_triangulation(tg) : tri6_from_triangulation(tg);
    // Materials follow descent: the builder wrote a single id, so replace it with each
    // parent's own. A refined mesh that lost its layers would be a different problem.
    fine.element_material.resize(fine.element_count);
    for (int e = 0; e < coarse.element_count; ++e)
        for (int k = 0; k < 4; ++k)
            fine.element_material[4 * e + k] = coarse.element_material[e];
    return fine;
}
